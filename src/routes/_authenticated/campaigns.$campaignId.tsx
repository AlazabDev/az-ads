...
import {
  ...
  Target, Lightbulb, TrendingUp, Languages, RefreshCw, X, ImagePlus, Search,
} from "lucide-react";
import { getCampaign, sendCampaignMessage, renameCampaign, deleteCampaign, listBrandAssets } from "@/lib/campaigns.functions";
...
function CampaignChat() {
  ...
  const sendMut = useMutation({
    mutationFn: (vars: { content: string; attachments: string[] }) =>
      sendFn({ data: { campaignId, content: vars.content, attachments: vars.attachments } }),
    onMutate: async (vars) => {
      ...
      const previewContent =
        ...
        (vars.attachments.length
          ? "\n\n" + vars.attachments.map((u) => `![مرفق](${u})`).join("\n")
          : "");
      qc.setQueryData(key, (old: typeof data) => old ? {
        ...old,
        messages: [...old.messages, { id: `tmp-${Date.now()}`, role: "user", content: previewContent, created_at: new Date().toISOString() }],
      } : old);
      return { prev };
    },
    onError: (_e, _v, ctx) => {
  ...
  const send = (text: string) => {
    ...
    setInput("");
    setAttached([]);
    sendMut.mutate({ content: trimmed, attachments: atts });
  };
  ...
  const toggleAttach = (url: string) => {
    setAttached((prev) =>
      prev.includes(url) ? prev.filter((u) => u !== url) : prev.length >= 6 ? prev : [...prev, url],
    );
  };
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-3 gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="inline-flex items-center gap-1 rounded-lg border border-border bg-card px-3 py-2 text-xs font-semibold text-muted-foreground hover:text-brand"
              title="أصول العلامة"
            >
          ...
          <form onSubmit={onSubmit} className="border-t border-border p-3 md:p-4 bg-card">
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {attached.map((url) => (
                  <div key={url} className="relative w-16 h-16 rounded-lg overflow-hidden border border-border">
                    ...
                    <button
                      type="button"
                      onClick={() => toggleAttach(url)}
                      className="absolute top-0 left-0 w-5 h-5 grid place-items-center bg-black/60 text-white text-xs"
            ...
            <div className="rounded-2xl border border-border bg-surface focus-within:border-brand transition p-2 flex items-end gap-2">
              <button
                type="button"
                onClick={() => setAssetsOpen(true)}
                className="shrink-0 inline-flex items-center justify-center w-11 h-11 rounded-xl border border-border bg-card text-muted-foreground hover:text-brand"
                aria-label="إرفاق من الأصول"
                title="إرفاق صور من الأصول"
              >
            ...
            <div className="text-[11px] text-muted-foreground mt-2 text-center">
              يعمل بواسطة Azure AI Foundry · GPT-5.5 · يدعم تحليل الصور المرفقة
            </div>
          </form>
        </section>
      </main>
...
function AssetsPicker({
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["brand-assets", search],
    queryFn: () => listFn({ data: { limit: 120, offset: 0, search: search || undefined } }),
  });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm grid place-items-center p-4" onClick={onClose}>
      <div
        dir="rtl"
        className="bg-card border border-border rounded-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div>
            <div className="font-bold text-brand">أصول العلامة</div>
            <div className="text-xs text-muted-foreground">اختر حتى 6 صور لإرفاقها مع رسالتك التالية</div>
          </div>
          <button onClick={onClose} className="text-muted-foreground hover:text-destructive" aria-label="إغلاق">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="p-3 border-b border-border">
          <div className="flex items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2">
            ...
            />
            <span className="text-[11px] text-muted-foreground">محدد: {selected.length}/6</span>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-3">
          {isLoading && (
            <div className="py-16 text-center text-muted-foreground">
              <Loader2 className="w-5 h-5 animate-spin inline mb-2" />
              <div className="text-sm">جاري تحميل الأصول…</div>
            </div>
          )}
          {error && <div className="text-sm text-destructive p-4">{(error as Error).message}</div>}
          {!isLoading && items.length === 0 && (
            <div className="py-16 text-center text-muted-foreground text-sm">لا توجد أصول مطابقة.</div>
          )}
            ...
            {items.map((it) => {
              ...
              return (
                <button
                  key={it.path}
                  type="button"
                  onClick={() => onToggle(it.url)}
                  className={`relative aspect-square rounded-lg overflow-hidden border transition ${
                    isSel ? "border-brand ring-2 ring-brand" : "border-border hover:border-brand/50"
                  }`}
                  title={it.name}
                >
                  <img src={it.url} alt={it.name} loading="lazy" className="w-full h-full object-cover" />
                  {isSel && (
                    <div className="absolute top-1 right-1 w-5 h-5 rounded-full bg-brand text-brand-foreground grid place-items-center">
        ...
        <div className="p-3 border-t border-border flex items-center justify-between">
          <div className="text-xs text-muted-foreground">
            المصدر: <code className="text-brand">Alazab-Ads/raw_images</code>
          </div>
          <button
            onClick={onClose}
            className="rounded-xl bg-brand text-brand-foreground px-4 py-2 text-sm font-semibold"
